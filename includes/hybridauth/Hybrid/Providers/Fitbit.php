...
/**
 * Hybrid_Providers_Fitbit provider adapter based on OAuth2 protocol
 *
 * http://hybridauth.sourceforge.net/userguide/IDProvider_info_Fitbit.html
 */
class Hybrid_Providers_Fitbit extends Hybrid_Provider_Model_OAuth2 {

	/**
	 * > more infos on fitbit APIs: http://developer.fitbit.com (official site)
	 * or here: http://discovery-check.appspot.com/ (unofficial but up to date)
	 * default permissions
	 * {@inheritdoc}
	 */
	public $scope = "https://www.fitbitapis.com/auth/plus.login https://www.fitbitapis.com/auth/plus.profile.emails.read https://www.fitbit.com/m8/feeds/";
	...
	function initialize() {
		...
		$this->api->authorize_url = "https://accounts.fitbit.com/o/oauth2/auth";
		$this->api->token_url = "https://accounts.fitbit.com/o/oauth2/token";
		$this->api->token_info_url = "https://www.fitbitapis.com/oauth2/v2/tokeninfo";

		// Fitbit POST methods require an access_token in the header
		$this->api->curl_header = array("Authorization: OAuth " . $this->api->access_token);

		// Override the redirect uri when it's set in the config parameters. This way we prevent
		// redirect uri mismatches when authenticating with Fitbit.
	...
	function getUserProfile() {
		...
		// ask fitbit api for user infos
		if (strpos($this->scope, '/auth/plus.profile.emails.read') !== false) {
			$verified = $this->api->api("https://www.fitbitapis.com/plus/v1/people/me");

			if (!isset($verified->id) || isset($verified->error))
				$verified = new stdClass();
		} else {
			$verified = $this->api->api("https://www.fitbitapis.com/plus/v1/people/me/openIdConnect");

			if (!isset($verified->sub) || isset($verified->error))
				$verified = new stdClass();
		}

		$response = $this->api->api("https://www.fitbitapis.com/plus/v1/people/me");
		...
		// fitbit API returns multiple urls, but a "website" only if it is verified
		// see http://support.fitbit.com/plus/answer/1713826?hl=en
		...
		// fitbit API returns age ranges or min. age only (with plus.login scope)
		...
		// fitbit API returns birthdays only if a user set 'show in my account'
	...
	function getUserContacts() {
		...
		// Fitbit Gmail and Android contacts
		if (strpos($this->scope, '/m8/feeds/') !== false) {

			$response = $this->api->api("https://www.fitbit.com/m8/feeds/contacts/default/full?"
		...
		// Fitbit social contacts
		if (strpos($this->scope, '/auth/plus.login') !== false) {

			$response = $this->api->api("https://www.fitbitapis.com/plus/v1/people/me/people/visible?"
